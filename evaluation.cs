...
namespace SP_Bewegungserkennung {
    class evaluation {
        ...
        //Darijas evaluation: evaluate each FSM
        ...
        public string evaluate2(int tresholdMultiplier, point variance) {
            ...
            //format
            for (int q = 0; q < shapeList.Count; q++) {
                sb.Append(q + 1).Append(";");
            }
            sb.Append("totalRecognised").Append(";").Append("totalTested").Append(";").Append("gesturesNotRecognised(no machine)").Append(";");
            sb.Append("\n");
        ...
        private int[] evaluateShape(Shape shape, List<FSM> machineList, StringBuilder sb) {
            ...
            foreach (Gesture g in shape.getGestures()) {
                ...
                if (recognisedMachineID < 0) {
                    if (!recogInThisIteration) gesturesNotRecognised++;
                    continue;
                }
            ...
            sb.Append(totalGesturesRecognised).Append(";").Append(totalGesturesTested).Append(";").Append(gesturesNotRecognised).Append(";").Append("\n");
